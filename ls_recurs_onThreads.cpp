...
void writeFiles(Folder* actualFolder, int level, int currentThreadID) {
    ...
    for (std::list<FInfo*>::iterator iter = actualFolder->child.begin(); iter != actualFolder->child.end(); iter++) {
        ...
        if (child->isFolder) {
            std::thread newThread(writeFiles, (Folder*)child, level + 1, currentThreadID + 1);
            actualThreads.push_back(std::move(newThread));
        }
    }

    for(std::thread & thread : actualThreads){
        thread.join();
    }
};
...
int main() {
    ...
    writeFiles(actualFolder, 1, currentThreadID);
